...
def main() -> None:
    ...
    ap.add_argument(
        "--config", required=True, help="training config YAML the checkpoint was saved under"
    )
    ap.add_argument("--raw-ckpt", required=True, help="raw torch_layers.state_dict() .pt")
    ap.add_argument(
        "--out", required=True, help="output artifact YAML path (weights land alongside)"
    )
    ap.add_argument(
        "--allow-partial",
        action="store_true",
    ...
    print(
        f"[conv] saved canonical artifact -> {args.out} (+ {os.path.splitext(args.out)[0]}.pt)",
        flush=True,
    )
